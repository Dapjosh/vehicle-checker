...
import type { InspectionReportSummary, InspectionCategory } from '@/lib/definitions';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Loader2, Search, ChevronsUpDown, Check, X, Download } from 'lucide-react';
...
export default function ReportsPage() {
    ...
    const fetchReports = useCallback(async (orgId: string) => {
        if (!orgId) return;
        setLoading(true);
        try {
    ...
    const filteredReports = useMemo(() => {
        return reports.filter(report => {
            ...
            const matchesSearch = report.vehicleRegistration.toLowerCase().includes(searchLower) ||
                                  report.driverName.toLowerCase().includes(searchLower);
            const matchesStatus = statusFilter ? report.finalVerdict === statusFilter : true;
    ...
    const handleExport = () => {
        if (!user?.orgId) {
            toast({ variant: 'destructive', title: 'Error', description: 'Could not determine organization.' });
            return;
        }

        startExportTransition(async () => {
            const [reportsResult, checklistResult] = await Promise.all([
                getAllReportsForExport(user.orgId),
                getChecklistAction(user.orgId),
            ]);

            if (!reportsResult.success || !reportsResult.data) {
                toast({ variant: 'destructive', title: 'Export Failed', description: reportsResult.error || 'Could not fetch reports for export.' });
                return;
            }
            if (!checklistResult.success || !checklistResult.data) {
                toast({ variant: 'destructive', title: 'Export Failed', description: checklistResult.error || 'Could not fetch checklist for headers.' });
                return;
            }
            ...
            const data = reportsResult.data.map(report => {
                ...
                allItems.forEach(itemTemplate => {
                    const reportItem = report.items?.find(i => i.id === itemTemplate.id);
            ...
            toast({ title: 'Export Successful', description: 'Your report data has been downloaded.' });
        });
    };
        ...
        <div className="flex min-h-screen w-full flex-col bg-background">
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
                <div className="mx-auto grid w-full max-w-6xl gap-8">
                     <div>
                        <h2 className="text-3xl font-bold tracking-tight">Inspection Reports</h2>
                        <p className="text-muted-foreground">
                            Browse, search, and filter all vehicle inspection reports for your organization.
                        </p>
                    </div>
                     <Card>
                        <CardHeader>
                                            ...
                                            <CommandList>
                                                ...
                                                {verdicts.map((v) => (
                                                    ...
                                                    onSelect={(currentValue) => {
                                                        setStatusFilter(currentValue === statusFilter ? "" : currentValue.toUpperCase())
                                                        setOpen(false)
                                                    }}
                                                    >
                                                    <Check
                                                        className={`mr-2 h-4 w-4 ${statusFilter === v.value ? "opacity-100" : "opacity-0"}`}
                                                    />
                                                    {v.label}
                                                    </CommandItem>
                                                ))}
