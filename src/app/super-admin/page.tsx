...
function CreateOrgForm({ onOrgCreated }: { onOrgCreated: () => void }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const result = await createOrganizationAndInvite(orgName, userEmail, user.uid);
            if (result.success && result.signupLink) {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><Building /> Onboard New Organization</CardTitle>
                <CardDescription>
                   Generate an invitation link for a new organization and its first user. The organization will be created once the user signs up.
                </CardDescription>
            </CardHeader>
             <form onSubmit={handleSubmit}>
                <CardContent className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input id="orgName" value={orgName} onChange={(e) => setOrgName(e.target.value)} placeholder="New Company Inc." required disabled={isPending || !!generatedLink}/>
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="userEmail">User's Email</Label>
                        <Input id="userEmail" type="email" value={userEmail} onChange={(e) => setUserEmail(e.target.value)} placeholder="user@newco.com" required disabled={isPending || !!generatedLink} />
                    </div>
                    ...
                    {generatedLink && (
                        <div className="space-y-2 pt-4">
                            ...
                            <div className="flex items-center gap-2">
                                <Input id="signupLink" readOnly value={generatedLink} className="bg-muted/50" />
                ...
                <CardFooter className="gap-4">
                     {!generatedLink ? (
                        <Button type="submit" disabled={isPending || !orgName || !userEmail}>
  ...
  const fetchOrganizations = useCallback(async () => {
      ...
      try {
        ...
        toast({
            ...
            description: 'There was a problem fetching the list of organizations. Please refresh the page.'
        });
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex min-h-screen w-full flex-col bg-background">
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        <div className="mx-auto grid w-full max-w-4xl gap-8">
          ...
          <Card>
             <CardHeader>
              <CardTitle className="flex items-center gap-2"><Building /> Existing Organizations</CardTitle>
              <CardDescription>View all organizations currently in the system.</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <div className="space-y-2">
                  {organizations.length > 0 ? (
                    ...
                    <p className="text-muted-foreground p-4 text-center">No organizations created yet.</p>
                  )}
                </div>
              )}
            </CardContent>
