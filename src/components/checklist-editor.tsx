...
function SortableItem({
  ...
  item: InspectionItem;
  setEditingItem: (
    data: { categoryId: string; item: InspectionItem } | null
  ) => void;
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center bg-background border-b last:border-b-0"
    >
      <button
        {...attributes}
        {...listeners}
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative bg-background">
      <button
        {...attributes}
        {...listeners}
        className="absolute left-1 top-1/2 -translate-y-1/2 p-3 z-10 cursor-grab active:cursor-grabbing"
      >
        <GripVertical className="h-5 w-5 text-muted-foreground" />
      </button>
...
export default function ChecklistEditor() {
  ...
  const updateCategories = (
    newCategories:
      | InspectionCategory[]
      | ((prev: InspectionCategory[]) => InspectionCategory[])
  ) => {
    setCategories((prev) => {
      const updated =
      ...
      >
        <DialogContent>
          <CategoryForm
            ...
            onSubmit={(name, icon) =>
              editingCategory &&
              handleUpdateCategory(editingCategory.id, name, icon)
            }
...
function AddCategoryDialog({
  ...
  onAddCategory: (name: string, icon: IconName) => void;
}) {
    ...
    <TooltipProvider>
      <form onSubmit={handleSubmit}>
        ...
        <DialogFooter>
          ...
          <Button type="submit" disabled={!name || !icon}>
            {buttonText}
          </Button>
        </DialogFooter>
      </form>
